...
SearchReplaceDialog::SearchReplaceDialog(GtkTextView *parent, GtkTextBuffer *parentBuffer)
    : m_targetTextView(parent), m_targetTextBuffer(parentBuffer), m_dialog(nullptr), m_searchActive(false) {
    CreateDialog();
}
...
void SearchReplaceDialog::CreateDialog() {
    ...
    gtk_window_set_transient_for(GTK_WINDOW(m_dialog), GTK_WINDOW(gtk_widget_get_ancestor(GTK_WIDGET(m_targetTextView), GTK_TYPE_WINDOW)));
    ...
    gtk_text_buffer_get_selection_bounds(m_targetTextBuffer, &m_searchStart, &m_searchEnd);
    if (gtk_text_iter_equal(&m_searchStart, &m_searchEnd)) {
        gtk_text_buffer_get_iter_at_mark(m_targetTextBuffer, &m_searchStart,
                                        gtk_text_buffer_get_insert(m_targetTextBuffer));
    }
}
...
bool SearchReplaceDialog::FindText(const std::string& text, bool caseSensitive, bool regex, bool forward) {
    ...
    gtk_text_buffer_get_bounds(m_targetTextBuffer, &start, &end);
    std::string bufferText = gtk_text_buffer_get_text(m_targetTextBuffer, &start, &end, TRUE);
    ...
    if (foundPos != std::string::npos) {
        ...
        gtk_text_buffer_get_iter_at_offset(m_targetTextBuffer, &m_searchStart, foundPos);
        gtk_text_buffer_get_iter_at_offset(m_targetTextBuffer, &m_searchEnd, foundPos + searchText.length());

        // Select the found text
        gtk_text_buffer_select_range(m_targetTextBuffer, &m_searchStart, &m_searchEnd);

        // Scroll to the found text
        gtk_text_view_scroll_to_iter(m_targetTextView, &m_searchStart, 0.1, FALSE, 0.0, 0.0);
...
void SearchReplaceDialog::HighlightMatch(const GtkTextIter& start, const GtkTextIter& end) {
    ...
    gtk_text_buffer_apply_tag_by_name(m_targetTextBuffer, "search_highlight", &start, &end);
}

void SearchReplaceDialog::ClearHighlight() {
    GtkTextIter start, end;
    gtk_text_buffer_get_bounds(m_targetTextBuffer, &start, &end);
    gtk_text_buffer_remove_tag_by_name(m_targetTextBuffer, "search_highlight", &start, &end);
}
...
void SearchReplaceDialog::ReplaceCurrent() {
    if (gtk_text_buffer_get_has_selection(m_targetTextBuffer)) {
        const char* replacement = gtk_editable_get_text(GTK_EDITABLE(m_replaceEntry));
        if (replacement) {
            gtk_text_buffer_delete_selection(m_targetTextBuffer, TRUE, TRUE);
            gtk_text_buffer_insert_at_cursor(m_targetTextBuffer, replacement, -1);
...
void SearchReplaceDialog::ReplaceAll() {
    ...
    while ((pos = bufferText.find(searchStr, pos)) != std::string::npos) {
        ...
        gtk_text_buffer_get_iter_at_offset(m_targetTextBuffer, &iterStart, pos);
        gtk_text_buffer_get_iter_at_offset(m_targetTextBuffer, &iterEnd, pos + searchStr.length());

        // Replace the text
        gtk_text_buffer_delete(m_targetTextBuffer, &iterStart, &iterEnd);
        gtk_text_buffer_insert(m_targetTextBuffer, &iterStart, replacement, -1);

        // Update buffer text for next search
        bufferText = gtk_text_buffer_get_text(m_targetTextBuffer, &start, &end, TRUE);
        if (!caseSensitive) {
            std::transform(bufferText.begin(), bufferText.end(), bufferText.begin(), ::tolower);
        }
