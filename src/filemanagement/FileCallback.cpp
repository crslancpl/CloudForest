...
void filemanagement::AddCallback(filemanagement::Event event, void (*callback)(GFile*, GFileInfo*)){
    switch (event) {
    case FILE_EVENT_FILE_CHOOSEN:
        file_choosen_callbacks.emplace(callback);
        break;
    case FILE_EVENT_FOLDER_CHOOSEN:
        folder_choosen_callbacks.emplace(callback);
        break;
    default:
        break;
    }
